  ...
  if (error && !society) {
    return (
      <div className="max-w-4xl mx-auto text-center py-12">
        ...
        <h2 className="text-xl font-semibold text-slate-900">Error Loading Society</h2>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-slate-900">{society.name}</h1>
            <Badge variant={society.active ? 'success' : 'warning'}>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium text-slate-900">{society.address}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-500">Pincode</p>
                  <p className="font-medium text-slate-900">{society.pincode}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-500">Created On</p>
                  <p className="font-medium text-slate-900">{new Date(society.createdAt).toLocaleDateString()}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-500">Total Units</p>
                  <p className="font-medium text-slate-900">{stats?.unitCount || 0}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              {society.emergencyContacts && society.emergencyContacts.length > 0 ? (
                  ...
                  {society.emergencyContacts.map((contact, i) => (
                    <div key={i} className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg border border-slate-100">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-medium text-indigo-600">{contact.role}</p>
                ...
                <div className="text-center py-6 bg-slate-50 rounded-lg border border-slate-100 border-dashed">
                  <p className="text-sm text-slate-500">No emergency contacts configured yet.</p>
                  <p className="text-xs text-amber-600 mt-1">Society cannot be activated until admin configures contacts.</p>
                </div>
              )}
            </CardContent>
          </Card>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-slate-50 rounded-lg border border-slate-100">
                  <div className="flex items-center gap-3 mb-2">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${society.active ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-200 text-slate-500'}`}>
                      <Activity className="w-4 h-4" />
                    </div>
                    <div>
                      <p className="font-medium text-slate-900">{society.active ? 'System Active' : 'System Suspended'}</p>
                      <p className="text-xs text-slate-500">{society.active ? 'Residents can login' : 'Access blocked'}</p>
                    </div>
                  </div>
                </div>
